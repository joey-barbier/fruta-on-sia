            ...
            if style == .cardBack {
                ZStack {
                    if let nutritionFact = ingredient.nutritionFact {
                        NutritionFact.Details(nutritionFact: nutritionFact)
                            .padding(.bottom, 70)
                    }
...
struct IngredientGraphic_Previews: PreviewProvider {
    ...
    static var previews: some View {
        Group {
            Ingredient.Graphic(ingredient: ingredient, style: .thumbnail)
                .frame(width: 180, height: 180)
                .previewDisplayName("Thumbnail")
            
            Ingredient.Graphic(ingredient: ingredient, style: .cardFront)
                .aspectRatio(0.75, contentMode: .fit)
                .frame(width: 500, height: 600)
                .previewDisplayName("Card Front")

            Ingredient.Graphic(ingredient: ingredient, style: .cardBack)
                .aspectRatio(0.75, contentMode: .fit)
                .frame(width: 500, height: 600)
