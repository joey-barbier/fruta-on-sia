...
extension Smoothie.Details {
    var container: some View {
        ZStack {
            ...
            ForEach(smoothie.menuIngredients) { measuredIngredient in
                ...
                Ingredient.Card(ingredient: measuredIngredient.ingredient, presenting: presenting, closeAction: deselectIngredient)
    ...
    var content: some View {
        VStack(spacing: 0) {
            Smoothie.Header(smoothie: smoothie)
                ...
                LazyVGrid(columns: [GridItem(.adaptive(minimum: 130), spacing: 16, alignment: .top)], alignment: .center, spacing: 16) {
                    ForEach(smoothie.menuIngredients) { measuredIngredient in
                        ...
                        Button(action: { select(ingredient: ingredient) }) {
                            Ingredient.Graphic(ingredient: measuredIngredient.ingredient, style: presenting ? .cardFront : .thumbnail)
        ...
        VStack(spacing: 0) {
            ...
            Group {
                if let account = model.account, account.canRedeemFreeSmoothie {
                    Smoothie.RedeemButton(action: redeemSmoothie)
                } else {
                    Order.PaymentButton(action: orderSmoothie)
                }
            }
...
struct SmoothieView_Previews: PreviewProvider {
    static var previews: some View {
        Group {
            NavigationView {
                Smoothie.Details(smoothie: .berryBlue)
            }
            
            ForEach([Smoothie.thatsBerryBananas, .oneInAMelon, .berryBlue]) { smoothie in
                Smoothie.Details(smoothie: smoothie)
