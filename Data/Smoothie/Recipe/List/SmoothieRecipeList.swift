...
// MARK: - Body
extension Smoothie.Recipe.List {
    var body: some View {
        List {
#if os(iOS)
            ...
            ForEach(smoothies) { smoothie in
                NavigationLink(tag: smoothie.id, selection: $model.selectedSmoothieID) {
                    Smoothie.Recipe.Details(smoothie: smoothie)
                        .environmentObject(model)
                } label: {
                    Smoothie.Row(smoothie: smoothie)
                        .padding(.vertical, 5)
                }
            }
        }
    ...
    var unlockButton: some View {
        Group {
            if let product = model.unlockAllRecipesProduct {
                Smoothie.Recipe.UnlockButton(
                    product: Smoothie.Recipe.UnlockButton.Product(for: product),
                    purchaseAction: { model.purchase(product: product) }
                )
            } else {
                Smoothie.Recipe.UnlockButton(
                    product: Smoothie.Recipe.UnlockButton.Product(
